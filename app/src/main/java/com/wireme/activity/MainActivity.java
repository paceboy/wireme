...
public class MainActivity extends Activity {
    ...
    private Device getAssignDevice() {
        Device device = null;
        int size = displays.size();
        Log.d(TAG, "dis size = " + size);
        for (int i = 0; i < size; i++) {
        // tochange,将199.102改为render的ip地址即可
            if (displays.get(i).getDevice().getIdentity().toString().contains("199.102")) {
                device = displays.get(i).getDevice();
            }
        }
        return device;
    ...
    private void playremote() {
        currentDevice = getAssignDevice();

        Log.d(TAG, "dis identity = " + currentDevice.getIdentity());
        avtService = currentDevice.findService(new UDAServiceType("AVTransport"));
        if (avtService != null) {
            cp = upnpService.getControlPoint();
            cp.execute(new SetAVTransportURI(avtService, "http://video19.ifeng.com/video07/2013/11/11/281708-102-007-1138.mp4", "NO METADATA") {
                @Override
                public void success(ActionInvocation invocation) {
                    ...
                    cp.execute(new Play(avtService) {
                        @Override
                        public void success(ActionInvocation invocation) {
                            Log.i(TAG, "PlayNewItem success:" + "we married ...");
                        }
    ...
    List<DeviceItem> displays = new ArrayList<>();
    ...
    public class DeviceListRegistryListener extends DefaultRegistryListener {
        ...
        public void remoteDeviceAdded(Registry registry, RemoteDevice device) {
            ...
            if (device.getType().getNamespace().equals("schemas-upnp-org")
                ...
                final DeviceItem display = new DeviceItem(device, device
                        ...
                        device.getDisplayString(), "(REMOTE) "
                        + device.getType().getDisplayString());
                Log.d(TAG, "remoteDeviceAdded device idtity = "+display.getDevice().getIdentity());
        ...
        public void localDeviceAdded(Registry registry, LocalDevice device) {
            ...
            if (device.getType().getNamespace().equals("schemas-upnp-org")
                    && device.getType().getType().equals("MediaRenderer")) {
                final DeviceItem displayl = new DeviceItem(device, device
                        .getDetails().getFriendlyName(),
                        device.getDisplayString(), "(LOCAL) "
                        + device.getType().getDisplayString());
                Log.d(TAG, "localDeviceAdded device idtity = "+displayl.getDevice().getIdentity());
                displays.add(displayl);
            }
        }
